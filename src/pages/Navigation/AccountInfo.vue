  ...
  <div>
    <q-page padding>
      ...
      <q-dialog v-model="showEditPhotoURL">
        <editPhotoURL @close="showEditPhotoURL = false"></editPhotoURL>
      </q-dialog>
      <!-- 名前の変更モーダル -->
      <q-dialog v-model="showEditName">
        <editName @close="showEditName = false" :userName.sync="userName" :userId="userId"></editName>
      </q-dialog>
      <!-- プロフィールの変更モーダル -->
...
export default {
  ...
  created() {
    if (this.usersPublicInfo[this.userId]) {
